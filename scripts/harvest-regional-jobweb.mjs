/**
 * scripts/harvest-regional-jobweb.mjs
 *
 * Sourcing genuine verified jobs for Ethiopia (ET), Zambia (ZA), and Ghana (GH)
 * from direct regional job networks (JobWebEthiopia, JobWebZambia, JobWebGhana).
 * 
 * Standards:
 *  - 100% verified real employers (Abay Bank, First Quantum Minerals, Ermak Trading, etc.)
 *  - Full authentic descriptions (> 100 characters)
 *  - Real source URLs and direct locations
...
const TARGET_CONFIGS = [
  {
    ...
    categories: [
      'https://jobwebethiopia.com/job-category/accounting-jobs-in-ethiopia/',
      'https://jobwebethiopia.com/job-category/administrativesecretarial-jobs-in-ethiopia/',
  ...
  },
  {
    ...
    categories: [
      ...
      'https://jobwebzambia.com/job-category/first-quantum-minerals-ltd-fqm-jobs-in-zambia/',
      'https://jobwebzambia.com/job-category/administrativesecretarial-jobs-in-zambia/',
      'https://jobwebzambia.com/job-category/constructionreal-estate-jobs-in-zambia/',
...
async function harvestPortalJobs() {
  console.log('================================================================');
  console.log('🌍 REGIONAL JOB HARVESTER: ETHIOPIA, ZAMBIA, GHANA');
  console.log('🎯 Direct verified employer vacancies');
  console.log('================================================================\n');
  ...
  for (const conf of TARGET_CONFIGS) {
    ...
    if (!cInfo) {
      console.log(`⚠️  Country code ${conf.countryCode} not found in database, skipping.`);
      continue;
    }
    ...
    for (const catUrl of conf.categories) {
      ...
      console.log(`  Processing Sector: ${catSlug}...`);

      // We crawl page 1 and page 2 for each category
      const pagesToCrawl = [catUrl, `${catUrl}page/2/`];
          ...
          for (const jobItem of links) {
            try {
              ...
              if (inserted) {
                countryInserted++;
                grandTotalInserted++;
                if (countryInserted % 5 === 0 || countryInserted === 1) {
    ...
    console.log(`==> Total verified jobs inserted for ${cInfo.name}: ${countryInserted}`);
  }

  console.log(`\n🎉 Grand total regional jobs inserted across ET, ZA, GH: ${grandTotalInserted}`);
  await sql.end();
}

harvestPortalJobs().catch(e => {
  console.error(e);
  process.exit(1);
});
