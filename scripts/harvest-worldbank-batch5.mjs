/**
 * scripts/harvest-worldbank-batch5.mjs
 *
 * Expands World Bank Open Procurement notices database:
 * Fetches batch 5 (os=750, rows=200) across all 12 countries.
...
async function harvestWorldBankBatch5() {
  console.log('================================================================');
  console.log('🌍 WORLD BANK OPEN PROCUREMENT EXPANSION HARVESTER (BATCH 5)');
  console.log('🎯 Sourcing offset 750 (200 notices per country) for all 12 nations');
  console.log('================================================================\n');
  ...
  for (const target of COUNTRIES) {
    ...
    console.log(`\n--- Sourcing batch 5 for ${target.name} (${target.code}) ---`);
    const url = `https://search.worldbank.org/api/v2/procnotices?format=json&project_ctry_name_exact=${encodeURIComponent(target.queryName)}&rows=200&os=750`;
    ...
    try {
      ...
      for (const item of notices) {
        ...
        try {
          ...
          if (inserted) {
            added++;
            totalInserted++;
            if (added % 25 === 0 || added === 1) {
              console.log(`    ✓ Inserted tender [${totalInserted}]: "${cleanTitle.slice(0, 60)}..."`);
            }
          }
        } catch (dbErr) {
          // ignore duplicate
      ...
      console.log(`  ✓ Added ${added} new verified tenders for ${target.name}.`);
      totalInserted += added;
    } catch (err) {
  ...
  console.log('\n================================================================');
  console.log(`🎉 BATCH 5 EXPANSION COMPLETE: +${totalInserted} verified tenders added.`);
  console.log('================================================================\n');

  await sql.end();
}

harvestWorldBankBatch5().catch((err) => {
  console.error('Fatal batch 5 error:', err);
  process.exit(1);
});
